...
zipItaly = "\d{5}" ##simple 5 digits in sequence
...
def checkAddress2(soups, vurl):
    ...
    try:
        ...
        locsW = ['location', 'posizione', 'address', 'indirizzo']
        urlsAdd = []
        ##Get additona links
        for link in inter:
        ...
        signsL = ['Via', 'Piazza', 'Accesso', 'Allea', 'Alinea', 'Alzaia', 'Androna', 'Angiporto', 'Arco', 'Archivolto', 'Arena', 'Argine', 'Bacino', 'Baluardo', 'Banchi','Barbarìa', 'Bastione', 'Bastioni', 'Belvedere', 'Bivio', 'Borgata', 'Borgo', 'Borgoloco', 'Cal', 'Calata', 'Calle', 'Callétta', 'Campiello', 'Campo', 'Canale', 'Cantone', 'Capo', 'Carraia', 'Carrara', 'Carrarone', 'Carro', 'Cascina', 'Case', 'Cavalcavia', 'Cavone', 'Chiasso', 'Chiassetto', 'Chiassuola', 'Circonvallazione', 'Circumvallazione', 'Claustro', 'Clivio', 'Clivo', 'Complanare', 'Contrà', 'Contra', 'Corsetto', 'Corsia', 'Corso', 'Corte', 'Corticella', 'Cortile', 'Costa', 'Crocicchio', 'Crosa', 'Cupa', 'Diramazione', 'Discesa', 'Distacco', 'Emiciclo', 'Erta', 'Estramurale', 'Fondaco', 'Fondamenta', 'Fondo', 'Fossa', 'Frazione', 'Galleria', 'Gradinata', 'Gradini', 'Gradoni', 'Granviale', 'Isola', 'Larghetto', 'Largo', 'Laterale', 'Lido', 'Lista', 'Litoranea', 'Località', 'Localita', 'Lungadige', 'Lungarno', 'Lungo', 'Masseria', 'Merceria', 'Molo', 'Mura', 'Parallela', 'Passaggio', 'Passeggiata', 'Passeggio', 'Passo', 'Pendice', 'Pendino', 'Pendio', 'Penninata', 'Piaggia', 'Piazzetta', 'Piscina', 'Ponte', 'Portico', 'Porto', 'Prato', 'Prolungamento', 'Quadrato', 'Raggio', 'Ramo', 'Rampa', 'Rampari', 'Rampe', 'Ratto', 'Recinto', 'Regione', 'Rettifilo', 'Regaste', 'Riello', 'Rione', 'Rio', 'Ripa', 'Riva', 'Riviera', 'Rondò', 'Rondo', 'Rotonda', 'Rua', 'Ruga', 'Rugheta', 'Sacca', 'Sagrato', 'Saia', 'Salita', 'Salizada', 'Scalea', 'Scalette', 'Scalinata', 'Scalone', 'Scesa', 'Sdrucciolo', 'Selciato', 'Sentiero', 'Slargo', 'Sopportico', 'Sotoportego', 'Sottoportico', 'Spalto', 'Spiaggia', 'Spianata', 'Spiazzo', 'Strada', 'Stradale', 'Stradetta', 'Stradone', 'Stretta', 'Stretto', 'Svoto', 'Supportico', 'Terrazza', 'Tondo', 'Traversa', 'Tratturo', 'Trazzera', 'Tresanda', 'Tronco', 'Vanella', 'Vallone', 'Vial', 'Viale', 'Vialone', 'Vicinale', 'Vicoletto', 'Vicolo', 'Vico', 'Viella', 'Vietta', 'Villaggio', 'Viottolo', 'Viuzza', 'Viuzzo', 'Vocabolo', 'Volti', 'Voltone', '  ', '.', ':', ';', ',', '\n', '\t', '\r']
        ##LIst with italian steert names
        ##List with city names found (in acse no address can be located)
        includedCities = []
    
        ##1. check for inclusion of Spanish zipcode    
        for soup in soups:
            ##get text
            ...
            ##Check for Italian zipcode address
            included = re.findall(" " + zipItaly + " ", text)
            included = [x.strip() for x in included]
            ##Remove duplicates
            ...
            if len(included) > 0:
                ...
                for incl in included:
                    ...
                    for posZip in posZips:
                        ...
                        ##detect if a country muncipality name occurs right of zip code text, could also be Spain
        ...
        ##2. Check findings based on municipality/city name (if no zipcode has been found)
        if len(address) == 0:
        ...
        ##3. postprocessing
        ##A. Remove subsets of longer text
        address2 = []
        for add in address:
        ...
        ##B. Address should have a number in text or s/n san numero
        address2b = []
        for add in address2:
        ...
        ##D. Check address for Spain or Espana (removes any etxra stuff added after that)
        address4 = []
        ItalyW = ['Italy', 'Italia', 'ITALY', 'ITALIA']
        for add in address3:
            ##Check for Spain
            included = list(filter(lambda x: re.findall(" " + x.upper() + " ", add), ItalyW))
            ##if italy is in address
            if len(included) > 0:
                posSpa = -1
                ##Check occurences
                for incl in included:
        ...
        if len(address4) == 0:
            ...
            ##if Spain is mentiond add the countris name (always)
            if inItaly:
                address4.append("Italia")
...
def checkPhone(soups, vurl):
    ...
    for soup in soups:
        ...
        ##1. Check land line numbers in Italy
        itaLandline1 = '\+39?\d{6,11}[\s]{1}|\+39?0\d{6,11}[\s]{1}|\0039?\d{6,11}[\s]{1}|\0039?0\d{6,11}[\s]{1}'
        phoneL = re.findall(itaLandline1, text1)
        ...
        ##2. Check mobile phone numbers in Italy (start with +39, 0039 or with 3)
        itaMobile1 = '\+39?[3]{1}\d{9}[\s]{1}|0039?[3]{1}\d{9}[\s]{1}|[3]{1}\d{9}[\s]{1}'
        phoneM = re.findall(itaMobile1, text1)    
        ##If nothing i found check soup
        ...
        ##3. Check for any numeric sequence starting with a + between spaces (length between 7 and 11 digits)
        ...
        ##compare findings, check if number starting with a + is found
        phone1b = []
        for ph in phone:
            ##Include numbers starting with + and zero
            if ph.startswith("+"):
                phone1b.append(ph)
            elif ph.startswith("0") or ph.startswith("3"): ##Could be a 0 or 3
                phone1b.append(ph)
...
def checkVAT(soups):
    ...
    for soup in soups:
        ...
        ##search for 11 digits, may be preceded by IT, IVA could close by
        itaVAT = '\s{1}[\d]{11}\s{1}|IT[\d]{11}\s{1}'
        VATs = re.findall(itaVAT, text1)
        ##Check result
        if len(VATs) == 0:
            ##check soup
            VATs = re.findall(itaVAT, str(soup)) ##risque as some Google codes may be found!
            text1 = str(soup)

        ##Check if findings are preceded by IVAL
        if len(VATs) > 0:
            for v in VATs:
                v = v.strip()
                start = 0
...
def getActivitiesText(text):
    ...
    if len(text) > 0:
        ...
        if not boolDrone:
            dronew  = ['droni', 'drone', ' rpas ', ' sapr ', ' uas ', ' uav ', ' unmanned ', ' senza pilota ', ' enav ']
            for w in dronew:
                if text.find(w) > -1:
        ...
        if not "manufacturing" in activ:
            manuf = [' manufactu', ' fabrica', 'produzion', 'fabbricazion']
            for w in manuf:
                if text.find(w) > -1:
        ...
        if not "distributing" in activ:
            distr = [' distribu', ' dealer ', ' rivendit']
            for w in distr:
                if text.find(w) > -1:
        ...
        if not "maintaining" in activ:
            maint = [' maintain', ' mantenimiento ', ' manutenzion', " repair ", " riparazion"]
            for w in maint:
                if text.find(w) > -1:
        ...
        if not "sales" in activ:
            sal = [" retail ", " shop ", " vendita ", " negozio "]
            for w in sal:
                if text.find(w) > -1:
                    activ.append("sales")
                    break
        ...
        if not "renting" in activ:
            rent = [" rent ", " renting ", " noleggio ", " locazion"]
            for w in rent:
                if text.find(w) > -1:
        ...
        if not "training" in activ:
            train = [" training ", " formazion", " course ", " corso ", " corsi "]
            for w in train:
                if text.find(w) > -1:
        ...
        if not "filming/imaging" in activ: ##photo
            film = ["filming ", " video", " riprese ", " audiovisual", "audiovisivo", " photo", " foto", " imaging ", " immagini "] ##?audiovisual include or not?
            for w in film:
                if text.find(w) > -1:
        ...
        if not "inspection" in activ:
            insp = [" survey", " mapping ", " inspection", " riliev", " mappatura ", " ispezione "] ##solutions, solutiones?
            for w in insp:
                if text.find(w) > -1:
                    activ.append("inspection")
                    break
            
        ##Check for dcomponents for drones
        if not "components" in activ:
            payl = [" component", " payload ", " cargo", " parachut", " camera", " accessor"]
            for w in payl:
                if text.find(w) > -1:
        ...
        if not "consultancy" in activ:
            mang = [" consulenz", " consulting ", " gestione aero"] 
            for w in mang:
                if text.find(w) > -1:
        ...
        if not "entertainment/race" in activ:
            ent = [" race ", " gara ", " show ", " spettacolo", " entertainment ", " intrattenimento ", " theat", " dancing ", " teatra", " ballare "]
            for w in ent:
                if text.find(w) > -1:
        ...
        if not "design"in activ:
            des = [" design ", " desino ", " integration", " integrazione"]
            for w in des:
                if text.find(w) > -1:
                    activ.append("design")
                    break
...
def getActivities(soups, vurl, descrip, name):
    ...
    if len(activ2) == 0:
        ...
        words = ['services', 'servici', 'activities', 'attività', 'attivita', ['what', 'do'], ['cosa', 'faccia'], ['cosa', 'fara']]
        ##words = ['nuestros servicios', 'our services']
...
def checkEcommerce(soups):
    ...
    words = ['shop', 'negozzio', 'cart', 'carrello', 'cestino', 'cesto', 'customer', 'client', 'cliente'] ##carro ?
...
def checkJobs(soups):
    jobs = False
    text = ""
    words = ['jobs', 'lavori', 'vacancy', 'vacante', 'vancancies']
...
def getRegionText(text, level = 'nuts2'):
    region= ""
    if not level == "nuts2" and not level == "nuts3" and not level == "province":
        level = "nuts2"
        ##only nuts2, nuts3 and province allowed
             
    ##get locatons from text provided
    if len(text) > 0:
        ##process text, replace . and , and :
        text = preprocessText(text)
        
        ##Check for Spanish zipcode in text
        included = re.findall(" " + zipItaly + " ", text)
        included = [x.strip() for x in included]
        included = list(set(included))
    
        ##1. Check if zipcode is included (no need to specially process text)
        if len(included) > 0:
            for incl in included:
                ##Get first 2 digits
                incl = incl[0:2]
                ...
                incl = int(incl)
                ##get rows from table
                rowM = municLC.loc[municLC['ZIP2digits'] == incl]
...
def checkInCountry(soups, address, phone, vurl):
    ...
    ##start with phone
    if len(phone) > 0:
        ##Check for occurence of +39 or 0039 Countrycode of Italy
        phones = phone.split(",")
        for ph in phones:
            ph = ph.strip()           
            ##Check for Spanish countrycode
            if ph.startswith("+39") or ph.startswith("0039"):
                ##Country found
                inCountry = True
                ##Stop search
                break
    ...
    if inCountry == None:
        ...
        if len(address) > 0:
            for add in address:
                ...
                add = " " +  add.strip() + " "
                ##print(add)
                ##Check for occurence of Spain in text, but beware of add with single españa word!!!(see address4 part of checkAddress2 function)
                included = list(filter(lambda x: re.findall(x, add), countryN))         
                if len(included) > 0 and not add == " italia ":
                    inCountry = True
                    break
            
            ##When Espania is not there but the address ends with a correct Spanish cityname (this is a valid address), its in the country
            if not inCountry:
            ...
            for soup in soups:
                ...
                ##Check for occurence of Spain in text
...
def checkIfCompany(soups, address, phone, VAT, descrip, vurl):
    ...
    if not boolComp:
        ...
        ##What is typical for a company?, MUST have an address, email and phone AND CONTACT PAGE OR ABOUT US PAGE (ergo multiple soups)
            ...
            compW = ["company", "business", "azienda", "impresa"]
            ##Check occurence
            ...
            try:                
                ...
                if len(str(soup)) > 1:            
                    ...
                    if len(pageSoups) > 0:
                        ...
                        ##12 get inCountry (checks for Italy Italia in address +39 in phone other wise in pageSoups)
...
if(len(sys.argv) > 1):
    ...
    print("Use 'python3 Script7_IniIT.py <filename.csv> to run program") 
...
if Continue:
    ...
    try: 
        ...
        if os.path.isfile("IT_munic_prov_names.csv"):
            ##Get city and nuts region list
            municLC = pandas.read_csv("IT_munic_prov_names.csv", sep = ";")
            ##Get municipalities
            municL = list(municLC['Municipality'])
            ##Append country name to municL
            municL.append("Italy")
            municL.append("Italia")
        else:
            print("City, nuts and zipcode file is not found; name = IT_munic_prov_names.csv")
            Continue = False
