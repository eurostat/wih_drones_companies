...
##Checked phone number regexs of Spanish zipcode
...
zipSpain = "(?:0[1-9]|[1-4]\d|5[0-2])\d{3}"
...
def checkAddress2(soups, vurl):
    ...
    try:
        ...
        locsW = ['location', 'ubicación', 'ubicacion', 'address']
        urlsAdd = []
        ##Get additona links
        for link in inter:
        ...
        signsL = ['Avenida', 'Av ', 'Avda ', 'Ave ', 'Barriada', 'Bda ', 'Barrio', 'Bo ', 'Bulevar', 'Boulevard', 'Blvd ', 'Pg ', 'Calle', 'C /', 'Cll ', 'Callejón', 'Callejon', 'Calzada', 'Camino', 'Cmo ', 'Cam ', 'Caminito', 'Cmt ', 'Carrera', 'Carretera', 'Ctra ', 'Carr ', 'Circunvalación', 'Colonia', 'Corredera', 'Costanilla' 'Cuesta', 'Diagonal', 'Eje', 'Glorieta', 'Malecón', 'Muelle', 'Pasaje', 'Pje ', 'Paseo', 'Pso ',  'Placeta', 'Placita', 'Plaza', 'Pla ', 'Pza ', 'Poblado', 'Puerta', 'Rambla', 'Retorno', 'Ronda', 'Rúa', 'Ruta', 'Senda', 'Transversal', 'Travesía', 'Urbanización', 'Urb ', 'Vereda', 'Ver ', 'Vía ', 'Viaducto', '  ', '.', ':', ';', ',', '\n', '\t', '\r']
        ...
        for soup in soups:
            ...
            ##Check for Spanish zipcode address
            included = re.findall(" " + zipSpain + " ", text)
        ...
        address4 = []
        SpainW = ['Spain', 'España', 'Espana', 'SPAIN', 'ESPAÑA', 'ESPANA']
        for add in address3:
            ##Check for Spain
            included = list(filter(lambda x: re.findall(" " + x.upper() + " ", add), SpainW))
            ##if spian is in address
            if len(included) > 0:
                posSpa = -1
                ##Check occurences
                for incl in included:
...
def checkPhone(soups, vurl):
    ...
    for soup in soups:
        ...
        ##1. Check land line numbers in Spain (double null numbers checked)
        spaLandline1 = '\+34?\d{9}[\s]{1}|\+34?0\d{9}[\s]{1}|0034?\d{9}[\s]{1}|0034?0\d{9}[\s]{1}'
        phoneL = re.findall(spaLandline1, text1)
        ...
        spaMobile1 = '\+34[6,7]{1}\d{7}[\s]{1}|0034[6,7]{1}\d{7}[\s]{1}|[0]{1}[6,7]{1}\d{7}[\s]{1}'
        phoneM = re.findall(spaMobile1, text1)    
        ##If nothing i found check soup
        ...
        for ph in phone:
            ...
            if ph.startswith("+"):
                phone1b.append(ph)
            elif ph.startswith("0"): ##Could be a 1 for toll free numbers
                phone1b.append(ph)
...
def checkVAT(soups):
    ...
    for soup in soups:
        ...
        ##search for 1 char with 9 digits, 9 digits with 1 char and 1 char with 8 digits and 1 char
        ##search for 1 char with 8 digits, 8 digits with 1 char and 1 char with 7 digits and 1 char
        spaVAT = '\s{1}[A-Z]{1}[\d]{8}\s{1}|\s{1}[\d]{8}[A-Z]{1}\s{1}|\s{1}[A-Z]{1}[\d]{7}][A-Z]{1}\s{1}'
        VATs = re.findall(spaVAT, text1)
        ##Check result
        if len(VATs) == 0:
            ##check soup
            VATs = re.findall(spaVAT, str(soup)) ##risque as some Google codes may be found!
            ##text1 = str(soup)
        ##SPANISH VAT NUMBERS ARE VERY SPECIFIC, no need to check preceding NIF/CIF/IVA
        ##Check findings
        if len(VATs) > 0:
            for v in VATs:
                if not v in VAT:
...
def getActivitiesText(text):
    ...
    if len(text) > 0:
        ...
        if not boolDrone:
            dronew  = [' dron', 'drone', ' rpas ', ' uas ', ' uav ', ' unmanned ', ' no tripulad ', ' aesa ']
            for w in dronew:
                if text.find(w) > -1:
        ...
        if not "manufacturing" in activ:
            manuf = [' manufactu', ' fabrica', ' producción ', ' produccion ']
            for w in manuf:
                if text.find(w) > -1:
        ...
        if not "maintaining" in activ:
            maint = [' mantenimiento ', ' maintenan', " repair ", " reparación ", " reparacion "]
            for w in maint:
                if text.find(w) > -1:
        ...
        if not "renting" in activ:
            rent = [" rent ", " renting ", " alquiler ", " arrendamiento "]
            for w in rent:
                if text.find(w) > -1:
        ...
        if not "training" in activ:
            train = [" training ", " formación", " formacion", " course ", " curso ", " cursos "]
            for w in train:
                if text.find(w) > -1:
        ...
        if not "filming/imaging" in activ: ##photo
            film = ["filming ", " video", " filmación ", " filmacion ", " audiovisual", " photo", " foto", " imaging ", " imágene", " imagene"] ##?audiovisual include or not?
            for w in film:
                if text.find(w) > -1:
        ...
        if not "inspection" in activ:
            insp = [" survey", " mapping ", " inspection", " riliev", " mapeo ", " inspección ", " inspeccion "] ##solutions, solutiones?
            for w in insp:
                if text.find(w) > -1:
        ...
        if not "components" in activ:
            payl = [" component", " payload ", " carga", " parachut", " paracaíd", " camera", " cámara", " camara", " accessor"]
            for w in payl:
                if text.find(w) > -1:
        ...
        if not "consultancy" in activ:
            mang = [" consultan", " consulting ", " gestión aero", " gestion aero"] 
            for w in mang:
                if text.find(w) > -1:
        ...
        if not "entertainment/race" in activ:
            ent = [" race ", " raza ", " show ", " espectáculo", " entertainment ", " entretenimiento ", " theat",  " teatro", " danza "]
            for w in ent:
                if text.find(w) > -1:
        ...
        if not "design"in activ:
            des = [" design ", " desiño ", " desino ", " integration", " integración", " integracion"]
            for w in des:
                if text.find(w) > -1:
                    activ.append("design")
                    break
...
def getActivitiesWords(text, words = 10):
    ...
    if len(text) > 0:
        ...
        ##0. Always check for drones synonyms in text
        dronew  = ['dron', 'drone', 'rpas', 'uas', 'uav', 'unmanned', 'tripulad', 'aesa']
...
def getActivities(soups, vurl, descrip, name):
    ...
    if len(activ2) == 0:
        ...
        words = ['services', 'servicios', 'activities', 'actividades', ['what', 'do'], ['qué', 'hacemos'], ['que', 'hacemos']]
...
def checkEcommerce(soups):
    ...
    words = ['shop', 'tienda', 'cart', 'carrito', 'bolsa', 'bolso', 'cesta', 'customer', 'client', 'cliente'] ##carro ?
...
def checkJobs(soups):
    jobs = False
    text = ""
    words = ['jobs', 'trabajos', 'vacancy', 'vacante', 'vancancies', 'vancantes']
            ...
            for incl in included:
                ...
                if len(rowM) > 0:
                    ...
                    if level == "nuts2":
                        ...
                        ##Two options remain
                        region1 = list(rowM['NUTS3a'])[0]
                        region2 = list(rowM['NUTS3b'])[0]
                        if region1 == region2:
                            region = region1
                        else:
        ...
        ##2. check for names
            ...
            ##2b Check for region names (nuts2 or nuts3)
                ...
                if level == "nuts2":
                    ...
                    ##Must be nuts3 names
                    nuts3 = list(set(list(municLC['NUTS3a']))) + list(set(list(municLC['NUTS3b'])))
                    ...
                    for incl in included:
                        ##Select rows
                        rowM1 = municLC.loc[municLC['NUTS3a'] == incl]
                        rowM2 = municLC.loc[municLC['NUTS3b'] == incl]
                        ##Check if something is found
                        region1 = ""
                        region2 = ""
                        if len(rowM1) > 0:
                            ##Could be two options, one can be empty
                            region1 = list(rowM1['NUTS3a'])[0]
                        if len(rowM2) > 0:
                            region2 = list(rowM2['NUTS3b'])[0]
...
def checkInCountry(soups, address, phone, vurl):
    ...
    ##start with phone
    if len(phone) > 0:
        ##Check for occurence of +34 or 0034 Countrycode of Spain
        phones = phone.split(",")
        for ph in phones:
            ph = ph.strip()           
            ##Check for Spanish countrycode
            if ph.startswith("+34") or ph.startswith("0034"):
                ##Country found
                inCountry = True
                ##Stop search
                break
    ...
    if inCountry == None:
        ...
        if len(address) > 0:
            for add in address:
                ...
                if len(included) > 0 and not add == " españa ":
                    inCountry = True
                    break
...
def checkIfCompany(soups, address, phone, VAT, descrip, vurl):
    ...
    if not boolComp:
        ...
        ##What is typical for a company?, MUST have an address, email and phone AND CONTACT PAGE OR ABOUT US PAGE (ergo multiple soups)
            ...
            compW = ["company", "business", "empresa", "operador", "negocio"]
            ##Check occurence
...
if(len(sys.argv) > 1):
    ...
    print("Use 'python3 Script7_IniES.py <filename.csv> to run program") 

##B. load ini-file if Continue
if Continue and not fileName1 == '':
    ## LOAD SETTING for ES file 
    fileName = "ES_es3.ini"
...
if Continue:
    ...
    try: 
        ...
        if os.path.isfile("ES_city_prov_names5b.csv"):
            ##Get city and nuts region list
            municLC = pandas.read_csv("ES_city_prov_names5b.csv", sep = ";")
            ##Get municipalities
            municL = list(municLC['Municipality'])
            ##Append country name to municL
            municL.append("Spain")
            municL.append("España")
            municL.append("Espana")
        else:
            print("City, nuts and zipcode file is not found; name = ES_city_prov_names5b.csv")
            Continue = False
    except:
        ##An error occured
...
if Continue:
    ...
    ##Store findings
    fileNameR = localDir + "/4_Result_" + country.upper() + "_3_extracted_info.csv"
