...
browser_wid="$(xdotool search --desktop 0 --sync --onlyvisible --class "${browser}" | head -n 1)"
...
if [[ "${browser}" == "firefox" ]]; then
    savefile_dialog_title="Opslaan"
else
    savefile_dialog_title="Opslaan"
fi

# Find window id for the "Save file" dialog box
savefile_wid="$(xdotool search --desktop 0 --name "$savefile_dialog_title" | head -n 1)"
